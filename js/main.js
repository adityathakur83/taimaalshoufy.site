  ...
  if (pills.length) {
    ...
    const applyFilter = (f) => {
      items.forEach(it => {
        ...
        // guarantee filtered-in content is visible even if its scroll-reveal
        // never fired while it was hidden
        if (show) {
          if (it.classList.contains("reveal")) it.classList.add("in");
          it.querySelectorAll(".reveal").forEach(r => r.classList.add("in"));
        }
      });
    };
    ...
    const initial = document.querySelector(".pill.active");
